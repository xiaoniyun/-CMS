var ajax=new AJAX();ajax.setcharset("utf-8");function reportErr(id){openWin("/"+sitePath+"err.php?id="+id,400,220,350,250,0)}
function AddFav(vid,uid)
{ajax.get("/"+sitePath+"include/ajax.php?action=addfav&id="+vid+"&uid="+uid,function(obj){if(obj.responseText=="err"){alert("请先登录!");location.href="/"+sitePath+"login.php";}else{alert("收藏成功!");}});}
function viewComment(id,page){var url;if(page.length==0){url=id;}else{url="/"+sitePath+"comment.php?id="+id+"&page="+page;}
ajax.get(url,function(obj){if(obj.responseText=="err"){set(document.getElementById("comment_list"),"<font color='red'>发生错误</font>")}else{set(document.getElementById("comment_list"),obj.responseText)}});}
function submitComment(id){if(document.getElementById("username").value.length<1){alert('请填写昵称');return false;}
if(document.getElementById("msg").value.length<1){alert('请填写内容');return false;}
ajax.postf(document.getElementById("f_comment"),function(obj){if(obj.responseText=="ok"){viewComment(id,1);alert('感谢您的评论!');}else if(obj.responseText=="validateerr"){alert('验证码错误，请点击验证码图片更新验证码！');}else if(obj.responseText=="havecomment"){alert('发布过于频繁，请稍后再操作！');}else if(obj.responseText=="ipcomment"){alert('您所在的ip不能评论');}else if(obj.responseText=="wordcomment"){alert('您的评论中有禁用词语，不能评论');}});}
function diggVideo(id,div){ajax.get("/"+sitePath+"include/ajax.php?id="+id+"&action=digg",function(obj){var returnValue=Number(obj.responseText)
...
function treadVideo(id,div){ajax.get("/"+sitePath+"include/ajax.php?id="+id+"&action=tread",function(obj){var returnValue=Number(obj.responseText)
...
function diggNews(id,div){ajax.get("/"+sitePath+"include/ajax.php?id="+id+"&action=diggnews",function(obj){var returnValue=Number(obj.responseText)
if(!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('成功顶贴！');}else if(obj.responseText=="err"){alert('顶贴失败')}else if(obj.responseText=="havescore"){alert('您已经执行过顶踩操作！')}});}
function treadNews(id,div){ajax.get("/"+sitePath+"include/ajax.php?id="+id+"&action=treadnews",function(obj){var returnValue=Number(obj.responseText)
if(!isNaN(returnValue)){set(document.getElementById(div),returnValue);alert('踩贴成功！');}else if(obj.responseText=="err"){alert('踩贴失败')}else if(obj.responseText=="havescore"){alert('您已经执行过顶踩操作！')}});}
...
switch(type){case 1:str=(pageNum==1)?'':"-"+pageNum;goUrl=regexpSplice(url,/(\/\/\S+\?\d+)[-]{0,1}\d*(\.html|\.htm|\.shtml|\.shtm|\.asp)/,str);break;case 2:if(url.lastIndexOf("/")==(url.length-1)){url+=listpagename}
...
break;case 5:str=(pageNum==1)?'':"-"+pageNum;goUrl=(url.split('-').length<2)?regexpSplice(url,/(\/\/\S+\d+?)(\.html|\.htm|\.shtml|\.shtm|\.asp|\.php)/,str):regexpSplice(url,/(\/\/\S+\d+?)-\d+(\.html|\.htm|\.shtml|\.shtm|\.asp)/,str);if(goUrl.indexOf('//')==-1){goUrl=regexpSplice(url,/(\/\/\S+\d+?)(\.html|\.htm|\.shtml|\.shtm|\.asp|\.php)/,str);}
break;case 6:str=(pageNum==1)?'':"-"+pageNum;goUrl=regexpSplice(url,/(\/\/\S+?)[-]{0,1}[\d]{0,1}(\.html|\.htm|\.shtml|\.shtm|\.asp|\.php)/,str);break;case 7:str=pageNum;goUrl=regexpSplice(url,/(\/\/\S+\?+?)\d+(\.html|\.htm|\.shtml|\.shtm|\.asp|\.php)/,str);break;}
location.href=goUrl;}
...
ajax.postf(document.getElementById("f_leaveword"),function(obj){if(obj.responseText=="ok"){viewLeaveWordList(1);alert('留言成功！');document.getElementById("m_content").value='';}else if(obj.responseText=="haveleave"){alert('发布过于频繁，请稍后再操作！');}else{alert('发生错误');}});}
function leaveWordgbook(){if(document.getElementById("vdcode").value.length<1){alert('验证码必须填写');return false;}
if(document.getElementById("m_content").value.length<1){alert('内容必须填写');return false;}
var obj=document.getElementById("m_content").value;if(!(/.*[\u4e00-\u9fa5]+.*$/.test(obj)))
{alert("留言内容请包含中文！");return false;}
return true;}
function leaveWordgbook2(){if(document.getElementById("m_content").value.length<1){alert('内容必须填写');return false;}
var obj=document.getElementById("m_content").value;if(!(/.*[\u4e00-\u9fa5]+.*$/.test(obj)))
{alert("留言内容请包含中文！");return false;}
return true;}
function getVideoHit(vid){ajax.get("/"+sitePath+"include/ajax.php?action=hit&id="+vid,function(obj){var result=obj.responseText
if(result=="err"){set(document.getElementById('hit'),'发生错误')}else{set(document.getElementById('hit'),result);}});}
function member()
{ajax.get("/"+sitePath+"include/ajax.php?action=member",function(obj){var result=obj.responseText;set(document.getElementById('seacms_member'),result);});}
function getNewsHit(nid){ajax.get("/"+sitePath+"include/ajax.php?action=hitnews&id="+nid,function(obj){var result=obj.responseText
if(result=="err"){set(document.getElementById('hit'),'发生错误')}else{set(document.getElementById('hit'),result);}});}
function markscore0(vd,d,t,s,l,ac){var alt=['很差','较差','还行','推荐','力荐'],url=ac=='news'?["/"+sitePath+"include/ajax.php?id="+vd+"&action=newsscore","/"+sitePath+"include/ajax.php?id="+vd+"&action=scorenews&score="]:["/"+sitePath+"include/ajax.php?id="+vd+"&action=videoscore","/"+sitePath+"include/ajax.php?id="+vd+"&action=score&score="],x=d,y=(Math.round(s/x*100)/100)||0,id='BT'+(new Date()).getTime();document.write('<div style="padding:5px 10px;border:1px solid #CCC">\
			...
			<div>');for(var i=0;i<=l;i++)document.write('<input type="radio" name="score" id="sint'+i+'" value="1" title="'+alt[parseInt(i/l*(alt.length-1))]+'"/><label for="sint'+i+'">'+i+'</label>');document.write('&nbsp;<input type="button" value=" 评分 " id="'+id+'" style="width:55px;height:21px"/>\
			</div>\
		</div>');document.getElementById(id).onclick=function(){for(var i=0;i<=l;i++)if(document.getElementById('sint'+i).checked)break;if(i>l){alert('你还没选取分数');return;}
ajax.get(url[1]+i,function(obj){if((''+obj.responseText).indexOf("havescore")!=-1){alert('你已经评过分啦');}else{document.getElementById('MARK_B4').innerHTML="评分成功！";alert('感谢你的参与!');}});this.disabled=true;}
if(new Date().toGMTString()!=new Date(document.lastModified).toGMTString())return ajax.get(url[0],function(obj){var a=obj.responseText
try{a.replace(/\[(\d+),(\d+),(\d+)\]/i,function($0,d,t,s){var x=parseInt(d),y=(Math.round(parseInt(s)/x*100)/100)||0;document.getElementById('MARK_B1').innerHTML=x;document.getElementById('MARK_B2').innerHTML=y;document.getElementById('MARK_B3').innerHTML=s;});}catch(ig){}});}
function showpf()
{document.getElementById('seacmsvpf1').style.display="none";document.getElementById('seacmsvpf2').style.display="inline";}
function markscore1(vd,d,t,s,l,ac){var alt=['很差','较差','还行','推荐','力荐'],src=['/'+sitePath+'pic/star0.gif','/'+sitePath+'pic/star1.gif'],url=ac=='news'?["/"+sitePath+"include/ajax.php?id="+vd+"&action=newsscore","/"+sitePath+"include/ajax.php?id="+vd+"&action=scorenews&score="]:["/"+sitePath+"include/ajax.php?id="+vd+"&action=videoscore","/"+sitePath+"include/ajax.php?id="+vd+"&action=score&score="],x=d,y=(Math.round(s/x*100)/100)||0,id='STAR'+(new Date()).getTime();document.write('<span id="'+id+'" style="padding:5px">');document.write('<span id="seacmsvpf1" onmouseover=showpf()></span>');document.write('<span id="seacmsvpf2">');for(var i=1;i<=l;i++){document.write('<img id="'+i+'" src="'+src[i<=y?0:1]+'" title="'+alt[parseInt(i/l*(alt.length-1))]+'" style="cursor:pointer">');}
document.write('</span>');document.write('&nbsp;<strong style="font-size:14px;color:red" id="MARK_B2"></strong>(<span id="MARK_B3"></span>)</span>');var dc=document.getElementById(id),im=dc.getElementsByTagName('img');for(var i=0;i<im.length;i++){im[i].onclick=function(){var x=parseInt(this.id);ajax.get(url[1]+x,function(obj){if((''+obj.responseText).indexOf("havescore")!=-1){alert('你已经评过分啦');}else{alert('感谢你的参与!');y=x;dc.onmouseout();}});}
im[i].onmouseover=function(){var x=parseInt(this.id);for(var i=0;i<im.length;i++)im[i].src=src[x>=parseInt(im[i].id)?0:1];}}
dc.onmouseout=function(){for(var i=0;i<im.length;i++)im[i].src=src[y>=parseInt(im[i].id)?0:1];document.getElementById('MARK_B2').innerHTML=y;document.getElementById('MARK_B3').innerHTML=y>0?alt[parseInt(y/l*(alt.length-1))]:'请选择';}
if(new Date().toGMTString()!=new Date(document.lastModified).toGMTString())return ajax.get(url[0],function(obj){var a=obj.responseText
try{a.replace(/\[(\d+),(\d+),(\d+)\]/i,function($0,d,t,s){var x=parseInt(d);y=(Math.round(parseInt(s)/x*100)/100)||0;dc.onmouseout();});}catch(ig){}});dc.onmouseout();}
function markNews2(vid,style,len){ajax.get("/"+sitePath+"include/ajax.php?action=npingfen&id="+vid,function(obj){var result=obj.responseText;result=result.split(",");num=result[0];sum=result[1];sc=result[2];if(style==1){document.getElementById('seacmsvpf2').style.display="none";id='STAR'+(new Date()).getTime();for(var ii=1;ii<=len;ii++){if(ii>sc){p=1;}else{p=0;}
document.getElementById('seacmsvpf1').innerHTML+='<img iid='+ii+' src="/pic/star'+p+'.gif" style="cursor:pointer">';}
document.getElementById('MARK_B2').innerHTML=sc;document.getElementById('MARK_B3').innerHTML=''+num+'次评分';}else{document.getElementById('MARK_B2').innerHTML=sc;document.getElementById('MARK_B3').innerHTML=sum;document.getElementById('MARK_B1').innerHTML=num;}});}
function markNews(vd,d,t,s,l,c){window['markscore'+(c==1?1:0)](vd,d,t,s,parseInt(l)<0?5:l,'news');}
function markVideo2(vid,style,len){ajax.get("/"+sitePath+"include/ajax.php?action=vpingfen&id="+vid,function(obj){var result=obj.responseText;result=result.split(",");num=result[0];sum=result[1];sc=result[2];if(style==1){document.getElementById('seacmsvpf2').style.display="none";id='STAR'+(new Date()).getTime();for(var ii=1;ii<=len;ii++){if(ii>sc){p=1;}else{p=0;}
document.getElementById('seacmsvpf1').innerHTML+='<img iid='+ii+' src="/pic/star'+p+'.gif" style="cursor:pointer">';}
document.getElementById('MARK_B2').innerHTML=sc;document.getElementById('MARK_B3').innerHTML=''+num+'次评分';}else{document.getElementById('MARK_B2').innerHTML=sc;document.getElementById('MARK_B3').innerHTML=sum;document.getElementById('MARK_B1').innerHTML=num;}});}
...
function loadSlide(w,h){var type=1
...
function addRemoteFavor(){ajax.get("/"+sitePath+"include/ajax.php?action=favorAjax&id="+play_vid+"&faction=add",function(obj){alert(obj.responseText)});}
